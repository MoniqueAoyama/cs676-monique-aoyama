...
def sigmoid(z):
    ...
    # Create an array to store the sigmoid results 
    result = np.empty_like(z, dtype=float)

    # Check which values are positive or negative 
    positive = z >= 0 
    negative = z < 0 
    ...
    # Different formula for negative values to avoid overflow 
...
def fit_gradient_descent(X_design, y, lr=LEARNING_RATE, n_iter=N_ITERATIONS):
    ...
    for _ in range(n_iter):
        ...
        # Make predictions with the current beta values 
        y_pred = predict_proba(X_design, beta)
